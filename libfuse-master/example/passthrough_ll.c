...
static int append_write(const char *content)
{
	FILE *dst;
	char file_name[] = "/out_put.txt";
	dst = fopen(file_name, "ab+");
	if (dst == NULL)
	{
		printf("open destination file failed\n");
		return 0;
	}
	fwrite(content, sizeof(content[0]), strlen(content), dst);
	fclose(dst);
	return 0;
}

static int append_uint64(uint64_t ino)
{
	char number[100];
	sprintf(number, "%llu\n", ino);
	append_write(number);
...
static int append_func(const char *_func)
{
	char func[100];
	sprintf(func, "-> %s\n", _func);
	append_write(func);
...
static void lo_mknod_symlink(fuse_req_t req,
							 fuse_ino_t parent,
							 const char *name,
							 mode_t mode,	   /* Type of file attribute bitmasks.  */
							 dev_t rdev,	   /* Type of device numbers.  */
							 const char *link) // *link 先不管。
{
	append_func(__func__);
	int res;
	int saverr;
	...
	// 查看该目录下是否有同名文件。
	// 一旦调用 lo_do_lookup，实际上等于：将 Inode 添加到缓存中。
	// 成功返回 0。
	saverr = lo_do_lookup(req, parent, name, &e);
	if (saverr) // 如果存在同名文件，退出。
		goto out;
	...
	// lo_do_lookup 创建了 Inode，这里创建了 entry。
	fuse_reply_entry(req, &e);
	return;
					...
					const char *name)
{
	...
	struct lo_data *lo = lo_data(req);
	struct lo_inode *inode = lo_inode(req, ino);
	struct fuse_entry_param e; // entry.
	char procname[64];
	int saverr;
	...
	res = linkat(AT_FDCWD, procname,
				 lo_fd(req, parent), name, AT_SYMLINK_FOLLOW);
	if (res == -1)
		goto out_err;
						  ...
						  int plus)
{
	...
	char *buf;
	char *p;
	size_t rem = size; // 4096，全部大小。
	int err;

	(void)ino;

	// buf 当然是用来接收 dir 的内容的。
	buf = calloc(1, size); // void *calloc(int num, int size)，创建 num * size 的大小。
	if (!buf)			   // 特殊情况，先不考虑。
	{
		err = ENOMEM;
		goto error;
	...
	while (1)
	{
		...
		{
			errno = 0;
			d->entry = readdir(d->dp); // 读出一个文件的信息。
			if (!d->entry)			   // 如果 readdir 返回 NULL，则表示没有读到或已经读完。
			{
				if (errno)
				{ // Error
					err = errno;
k					goto error;
		...
		if (entsize > rem) // 每次调用 readdir 或 readdirplus 都有 size 大小限制，当内容太多，耗尽了 size，
...
static void lo_readdir(fuse_req_t req,
					   fuse_ino_t ino,
					   size_t size,
					   off_t offset,
					   struct fuse_file_info *fi)
					  ...
					  struct fuse_file_info *fi)
{
	...
	err = lo_do_lookup(req, parent, name, &e); // 新 Inode 加入缓存，填充。
	if (err)
		fuse_reply_err(req, err);
	else
...
static void lo_statfs(fuse_req_t req, fuse_ino_t ino)
{
	...
	res = fstatvfs(lo_fd(req, ino), &stbuf); // <---- 主要是调用该方法。
	if (res == -1)
